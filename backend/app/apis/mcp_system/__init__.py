...
# Router with appropriate tags for MCP endpoints
router = APIRouter(tags=["mcp-system"])
...
@router.get("/mcp/status2", tags=["mcp"])
def get_mcp_status_endpoint2():
...
@router.post("/mcp/activate2", tags=["mcp"])
def activate_mcp_endpoint2(request: MCPActivationRequest):
...
def get_mcp_tools_endpoint():
    ...
    log_mcp_access("get_mcp_tools")
    return {
        "tools": get_available_mcp_tools(),
        "categories": {
            "clients": "Gestión de Clientes",
            "progress": "Monitoreo de Progreso",
            "training": "Gestión de Programas de Entrenamiento",
            "nutrition": "Gestión de Planes de Nutrición",
            "agent": "Sistema Inteligente Agéntico",
            "communication": "Comunicación y Monitoreo",
            "analysis": "Análisis y Reportes"
        }
    }

@router.get("/mcp/access-logs", tags=["mcp"])
def get_mcp_access_logs_endpoint():
    """Obtiene los registros de acceso al MCP"""
    logs = db.storage.json.get(MCP_LOGS_KEY, default=[])
    log_mcp_access("get_mcp_access_logs")
    return {
        "logs": logs[-100:], # Returns only the last 100 logs
        "total": len(logs)
    }

@router.post("/mcp/settings", tags=["mcp"])
def update_mcp_settings_endpoint(settings: dict = Body(...)):
    """Actualiza la configuración del MCP"""
    current_settings = get_mcp_settings()
    ...
    save_mcp_settings(current_settings)
    log_mcp_access("update_mcp_settings")
